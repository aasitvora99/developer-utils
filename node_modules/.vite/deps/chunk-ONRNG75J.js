...
// node_modules/mermaid/dist/styles-9a916d00.js
var parser = function() {
...
var ClassMember = class {
  constructor(input, memberType) {
    ...
    this.visibility = "";
    this.classifier = "";
    const sanitizedInput = sanitizeText$2(input, getConfig());
...
var splitClassNameAndType = function(_id) {
  const id = common$1.sanitizeText(_id, getConfig());
  let genericType = "";
  let className = id;
  if (id.indexOf("~") > 0) {
...
var setClassLabel = function(_id, label) {
  const id = common$1.sanitizeText(_id, getConfig());
  if (label) {
...
var addClass = function(_id) {
  const id = common$1.sanitizeText(_id, getConfig());
  const { className, type } = splitClassNameAndType(id);
  if (Object.hasOwn(classes, className)) {
    return;
  }
  const name = common$1.sanitizeText(className, getConfig());
  classes[name] = {
    id: name,
    type,
    label: name,
...
var addRelation = function(relation) {
  ...
  relation.relationTitle1 = common$1.sanitizeText(relation.relationTitle1.trim(), getConfig());
  relation.relationTitle2 = common$1.sanitizeText(relation.relationTitle2.trim(), getConfig());
...
var addNote = function(text, className) {
  const note = {
...
var getTooltip = function(id, namespace) {
  if (namespace) {
    return namespaces[namespace].classes[id].tooltip;
  }
  return classes[id].tooltip;
};
var setLink = function(ids, linkStr, target) {
...
var setClickFunc = function(_domId, functionName, functionArgs) {
  const domId = common$1.sanitizeText(_domId, getConfig());
  const config = getConfig();
...
var setupToolTips = function(element) {
  ...
  if ((tooltipElem._groups || tooltipElem)[0][0] === null) {
    tooltipElem = select_default("body").append("div").attr("class", "mermaidTooltip").style("opacity", 0);
  }
  const svg = select_default(element).select("svg");
  const nodes = svg.selectAll("g.node");
  nodes.on("mouseover", function() {
    const el = select_default(this);
...
var addClassesToNamespace = function(id, classNames) {
  if (namespaces[id] === void 0) {
    return;
  }
...
var setCssStyle = function(id, styles2) {
...
var getStyles = (options) => `g.classGroup text {
...
//# sourceMappingURL=chunk-ONRNG75J.js.map
