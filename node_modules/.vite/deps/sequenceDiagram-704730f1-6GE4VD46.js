...
import {
  ...
  setDiagramTitle,
  utils
} from "./chunk-5WRDZJBC.js";
import {
  __toESM
} from "./chunk-PR4QN5HX.js";

// node_modules/mermaid/dist/sequenceDiagram-704730f1.js
...
var addActor = function(id, name, description, type) {
...
var addMessage = function(idFrom, idTo, message, answer) {
...
var addSignal = function(idFrom, idTo, message = { text: void 0, wrap: void 0 }, messageType, activate = false) {
...
var hasAtLeastOneBox = function() {
  return state.records.boxes.length > 0;
};
var hasAtLeastOneBoxWithTitle = function() {
...
var getCreatedActors = function() {
  return state.records.createdActors;
};
var getDestroyedActors = function() {
  return state.records.destroyedActors;
};
var getActor = function(id) {
...
var showSequenceNumbers = () => state.records.sequenceNumbersEnabled;
...
var addNote = function(actor, placement, message) {
  const note = {
    actor,
    placement,
    message: message.text,
...
var addLinks = function(actorId, text) {
...
var addALink = function(actorId, text) {
  const actor = getActor(actorId);
  try {
    const links = {};
    let sanitizedText = sanitizeText$2(text.text, getConfig());
...
var addProperties = function(actorId, text) {
  const actor = getActor(actorId);
  try {
    let sanitizedText = sanitizeText$2(text.text, getConfig());
...
var addDetails = function(actorId, text) {
  const actor = getActor(actorId);
...
var actorCnt = -1;
var fixLifeLineHeights = (diagram2, actors, actorKeys, conf2) => {
  if (!diagram2.select) {
    return;
  }
...
var drawActorTypeParticipant = async function(elem, actor, conf2, isFooter) {
...
var drawActorTypeActor = async function(elem, actor, conf2, isFooter) {
  const actorY = isFooter ? actor.stopy : actor.starty;
  const center = actor.x + actor.width / 2;
...
var drawActor = async function(elem, actor, conf2, isFooter) {
  switch (actor.type) {
    case "actor":
...
var drawBox = async function(elem, box, conf2) {
...
var drawLoop = async function(elem, loopModel, labelText, conf2) {
  const {
    boxMargin,
    boxTextMargin,
    labelBoxHeight,
...
var drawBackgroundRect2 = function(elem, bounds2) {
...
var drawNote = async function(elem, noteModel) {
...
var drawMessage = async function(diagram2, msgModel, lineStartY, diagObj) {
...
var addActorRenderingData = async function(diagram2, actors, createdActors, actorKeys, verticalPos, messages, isFooter) {
  let prevWidth = 0;
  let prevMargin = 0;
  let prevBox = void 0;
  let maxHeight = 0;
...
var drawActors = async function(diagram2, actors, actorKeys, isFooter) {
  if (!isFooter) {
    for (const actorKey of actorKeys) {
      const actor = actors[actorKey];
...
var drawActorsPopup = function(diagram2, actors, actorKeys, doc) {
  let maxHeight = 0;
  let maxWidth = 0;
  for (const actorKey of actorKeys) {
    const actor = actors[actorKey];
...
var activationBounds = function(actor, actors) {
...
var draw = async function(_text, id, _version, diagObj) {
...
var buildNoteModel = async function(msg, actors, diagObj) {
...
var buildMessageModel = function(msg, actors, diagObj) {
  if (![
    diagObj.db.LINETYPE.SOLID_OPEN,
    diagObj.db.LINETYPE.DOTTED_OPEN,
    diagObj.db.LINETYPE.SOLID,
...
var calculateLoopBounds = async function(messages, actors, _maxWidthPerActor, diagObj) {
  const loops = {};
  const stack = [];
...
//# sourceMappingURL=sequenceDiagram-704730f1-6GE4VD46.js.map
